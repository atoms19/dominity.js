...
class finder{
  ...
  addChild(){
    Array.from(arguments).forEach((child)=>{
      ...
      if(child.finderElem){
        this.elem.appendChild(child.org)
      }else{
 ...
 clone(target = null, condition = true) {

   new finder(this.elem.cloneNode(condition), true).addTo(target)
 ...
 contains(nod){
   if(nod.finderElem){
   return this.elem.contains(nod.org)
   }else{
